  ...
  return (
    <>
      <div className="absolute z-0 -top-32 hidden lg:block">
        <svg className="max-w-full" xmlns="http://www.w3.org/2000/svg" width="1410" height="1267" viewBox="0 0 1410 1267" fill="none">
          ...
          <section className="flex flex-row items-center justify-between relative my-24">
            <div className="flex-1">
              <h1 className="text-white font-bold text-3xl lg:text-7xl">
                <span className="bg-gradient-to-r from-cyan-400 to-orange-400 bg-clip-text text-transparent">Search and Buy</span> Secure
                <br /> Property or Land Here!
              </h1>
              <p className="text-gray-500 lg:text-lg">With blockchain technology, now you can do property transactions effortlessly and completed within 7 days or less.</p>
                ...
                <div className="flex mb-10 lg:mb-0 lg:mr-5">
                  ...
                  <button
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="25" viewBox="0 0 24 25" fill="none">
                      <path d="M11.552 22.25C5.9254 22.25 1.34448 17.65 1.34448 12C1.34448 6.35 5.9254 1.75 11.552 1.75C17.1785 1.75 21.7594 6.35 21.7594 12C21.7594 17.65 17.1785 22.25 11.552 22.25ZM11.552 3.25C6.74199 3.25 2.83826 7.18 2.83826 12C2.83826 16.82 6.74199 20.75 11.552 20.75C16.3619 20.75 20.2656 16.82 20.2656 12C20.2656 7.18 16.3619 3.25 11.552 3.25Z" fill="#6F6F73" />
                      <path d="M22.0088 23.2499C21.8196 23.2499 21.6304 23.1799 21.481 23.0299L19.4893 21.0299C19.2005 20.7399 19.2005 20.2599 19.4893 19.9699C19.7781 19.6799 20.2561 19.6799 20.5449 19.9699L22.5366 21.9699C22.8254 22.2599 22.8254 22.7399 22.5366 23.0299C22.3872 23.1799 22.198 23.2499 22.0088 23.2499Z" fill="#6F6F73" />
                    </svg>
                  </button>
                </div>
                <div className="flex space-x-5 overflow-x-auto">
                  <button
                    className={`text-white font-bold rounded-lg py-3 px-5 ${viewModel.category == null ? "bg-gradient-to-r from-cyan-400 to-orange-400" : "border-2 from-cyan-400 "}`}
                    onClick={() => {
                  ...
                  <button
                    className={`text-white font-bold rounded-lg py-3 px-5 ${viewModel.category == "used" ? "bg-gradient-to-r from-cyan-400 to-orange-400" : "border-2 from-cyan-400 "}`}
                    onClick={() => {
                  ...
                  <button
                    className={`text-white font-bold rounded-lg py-3 px-5 ${viewModel.category == "new" ? "bg-gradient-to-r from-cyan-400 to-orange-400" : "border-2 from-cyan-400 "}`}
                    onClick={() => {
                  ...
                  <button
                    className={`text-white font-bold rounded-lg py-3 px-5 ${viewModel.category == "land" ? "bg-gradient-to-r from-cyan-400 to-orange-400" : "border-2 from-cyan-400 "}`}
                    onClick={() => {
            ...
            {viewModel.isLoading ? (
              ...
              <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {viewModel.properties.map((row) => (
                  <Link href={`/properties/${row.id}`} key={row.id} className="bg-zinc-900 rounded-lg p-5">
                    <div className="relative h-[20rem] w-full">
                      <img className="rounded-lg object-cover" src={row.image[0]} alt="Image" fill />
                      <span className="absolute top-3 left-3 bg-orange-400 py-1 px-3 rounded-lg text-white">{row.category.hasOwnProperty("used") ? "Used Home" : "New Home"}</span>
                    </div>
                    <h3 className="text-white text-xl lg:text-2xl mt-3 mb-2">{row.name}</h3>
                    <p className="text-gray-500 flex flex-row mb-3">
                      <img className="mr-2" src="/svg/point.svg" alt="Point" width={15} height={15} />
                      JL.Jeruk, Jakarta Selatan
                    </p>
                    <img src="/svg/break-line.svg" alt="Break Line" width={500} height={25} />
                    ...
                    <p className="text-cyan-400 text-lg lg:text-2xl">{Number(row.price)} ICP</p>
                  </Link>
                ))}
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center gap-y-5">
                ...
                <div className="flex flex-col items-center">
                  <p className="font-bold text-[24px] text-[#F3F3F3]">Result Not Found</p>
                  <p className="font-bold text-[16px] text-[#6F6F73]">Please try again with another keywords or maybe use generic term</p>
                </div>
              </div>
            )}
          </section>
